...
feature 'Ecm::Courses::Course admin' do
  ...
  def set_resource_path
    @resource_path = "ecm_courses_courses"
  end

  def set_resource_class
    @resource_class = Ecm::Courses::Course
  end
  ...
  def fill_new_form
    select @course_category.name,            :from => 'ecm_courses_course[ecm_courses_course_category_id]'
    select I18n.locale.to_s,            :from => 'ecm_courses_course[locale]'
    fill_in "ecm_courses_course[name]", :with => "Example course"
    # select 'textile',                            :from => 'ecm_courses_course_category[markup_language]'
  end

  def fill_edit_form
    fill_in "ecm_courses_course[name]", :with => "An updated course"
  end
