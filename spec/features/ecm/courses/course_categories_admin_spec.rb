...
feature 'Ecm::Courses::CourseCategory admin' do
  ...
  def set_resource_path
    @resource_path = "ecm_courses_course_categories"
  end

  def set_resource_class
    @resource_class = Ecm::Courses::CourseCategory
  end
  ...
  def fill_new_form
    select I18n.locale.to_s,                     :from => 'ecm_courses_course_category[locale]'
    fill_in "ecm_courses_course_category[name]", :with => "Example course category"
    # select 'textile',                            :from => 'ecm_courses_course_category[markup_language]'
  end

  def fill_edit_form
    fill_in "ecm_courses_course_category[name]", :with => "An updated course category"
  end
