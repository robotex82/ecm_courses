...
feature 'Ecm::Courses::CourseDate admin' do
  ...
  def set_resource_path
    @resource_path = "ecm_courses_course_dates"
  end
  ...
  def set_index_check_column
    @index_check_column = :duration_in_minutes
  end
  ...
  def fill_new_form
    select @course.to_s, :from => 'ecm_courses_course_date[ecm_courses_course_id]'
    # select_datetime should be happening here, but there is already a default value.
    # select 'textile',                            :from => 'ecm_courses_course_date[markup_language]'
  end

  def fill_edit_form
    fill_in "ecm_courses_course_date[description]", :with => "An updated course date"
  end
  ...
  describe 'index' do
    ...
    scenario "should show the resources" do
      @resources.each do |resource|
        page.body.should include(resource.send(@index_check_column.to_sym).to_s)
      end
    end # scenario
  end # describe 'index'
end # feature
